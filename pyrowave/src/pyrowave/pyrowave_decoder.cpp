...
void DecoderInput::flush()
{
	auto fl = [](buffer_allocation & b) {
		if (VmaAllocation(b) == nullptr)
			return;
		auto props = b.properties();
		if ((props & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) &&
		    !(props & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT))
			vmaFlushAllocation(vk_allocator::instance(), b, 0, VK_WHOLE_SIZE);
	};
	fl(dequant_offset_buffer);
	fl(dequant_staging);
	fl(payload_data);
	fl(payload_staging);
}
        ...
        ds_pool(nullptr)
{
	...
	if (!ops)
	{
		auto sg = phys_dev.getProperties2<vk::PhysicalDeviceProperties2, vk::PhysicalDeviceSubgroupProperties>();
		ops = sg.get<vk::PhysicalDeviceSubgroupProperties>().supportedOperations;
	}
	...
	// Compatibility backfill for Vulkan 1.1/1.2 devices (e.g. Adreno): such drivers
	// do not populate the Vulkan13/Vulkan12 aggregate structs, but they DO populate
	// the equivalent extension structs. Copy the fields the codec relies on.
	if (prop13.maxSubgroupSize == 0)
	{
		auto sg = phys_dev.getProperties2<vk::PhysicalDeviceProperties2, vk::PhysicalDeviceSubgroupSizeControlProperties>();
		const auto & sgp = sg.get<vk::PhysicalDeviceSubgroupSizeControlProperties>();
		prop13.minSubgroupSize = sgp.minSubgroupSize;
		prop13.maxSubgroupSize = sgp.maxSubgroupSize;
		prop13.maxComputeWorkgroupSubgroups = sgp.maxComputeWorkgroupSubgroups;
		prop13.requiredSubgroupSizeStages = sgp.requiredSubgroupSizeStages;
	}
	{
		auto ef = phys_dev.getFeatures2<vk::PhysicalDeviceFeatures2, vk::PhysicalDevice8BitStorageFeatures, vk::PhysicalDeviceShaderFloat16Int8Features>();
		const auto & f8 = ef.get<vk::PhysicalDevice8BitStorageFeatures>();
		const auto & ff16 = ef.get<vk::PhysicalDeviceShaderFloat16Int8Features>();
		if (!feat12.storageBuffer8BitAccess) feat12.storageBuffer8BitAccess = f8.storageBuffer8BitAccess;
		if (!feat12.uniformAndStorageBuffer8BitAccess) feat12.uniformAndStorageBuffer8BitAccess = f8.uniformAndStorageBuffer8BitAccess;
		if (!feat12.shaderFloat16) feat12.shaderFloat16 = ff16.shaderFloat16;
		if (!feat12.shaderInt8) feat12.shaderInt8 = ff16.shaderInt8;
	}
		...
		for (int level = 0; level < DecompositionLevels; level++)
		{
			...
			for (int comp = 0; comp < 3; comp++)
			{
				...
				if (comp < 2)
				{
					...
					view_info.image = vk::Image(static_cast<VkImage>(fragment.levels[level].vert[1][comp]));
			...
			for (int comp = 0; comp < NumComponents; comp++)
			{
				...
				for (int band = 0; band < NumFrequencyBandsPerLevel; band++)
				{
					...
					{
						image = &fragment.levels[level].horiz[comp];
						view_info.image = vk::Image(static_cast<VkImage>(*image));
